  ...
  return (
    <Component
      className={cn(
        loading
          ? cn(
              "animate-pulse rounded-md bg-gray-300 text-transparent dark:bg-slate-900",
              loadingClassName
            )
          : "",
        className
...
const SkeletonText: React.FC<SkeletonBaseProps & { invisible?: boolean }> = ({
  ...
  return (
    <span
      className={cn(
        `inline-block animate-pulse rounded-md bg-gray-300 empty:before:inline-block empty:before:content-['']`,
        invisible ? "invisible" : "",
        "dark:bg-slate-700",
        className
      )}
